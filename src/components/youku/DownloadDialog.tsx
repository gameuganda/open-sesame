...
export function DownloadDialog({ open, onClose, sources, baseName }: Props) {
  ...
  /** A file far smaller than the advertised size is the provider's promo clip. */
  const isPromo = (source: StreamSource) => {
    const real = probed[source.id];
    if (!real) return false;
    return real < 80 * 1024 * 1024 && (source.bytes ?? 0) > real * 4;
  };
  ...
  // Keep every provider file, including alternate codecs at the same quality.
  const videos = [...sources].sort(
    (a, b) => a.resolution - b.resolution || String(a.codec).localeCompare(String(b.codec)),
  );
        ...
        <div className="max-h-[65vh] overflow-y-auto px-5 pb-5">
          ...
          {videos.length ? (
            <div className="mt-2 grid grid-cols-3 gap-3">
              {videos.map((source) => {
                const promo = isPromo(source);
                const label = `${baseName}.${source.resolution || "auto"}p`;
                return promo ? (
                  <div key={source.id} className={`${tile} cursor-not-allowed opacity-50`}>
                    <span className="text-sm font-bold text-foreground">
                      {source.resolution ? `${source.resolution}P` : "AUTO"}
                    </span>
                    <span className="text-[10px] text-muted-foreground">Stream only</span>
                  </div>
                ) : (
                  <a
                    key={source.id}
                    href={mediaDownloadUrl(source.url, label)}
                    download={mediaDownloadName(label)}
                    onClick={guard}
                    className={tile}
                  >
                    <span className="text-sm font-bold text-foreground">
